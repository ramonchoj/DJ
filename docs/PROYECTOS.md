...
## 24. "No veo forma de poner un vídeo" — modo YouTube bloqueado (2026-09-12) — v3.4.1
**Causa:** `iniciarYouTube` hacía `await motorYt.preparar()` (espera el `onReady` de los dos reproductores) antes de dibujar la biblioteca; con internet lento ese aviso no llega y quedaban solo cajas negras sin botón. Además `preparar()` no tenía timeout (se colgaba para siempre) y el único camino para agregar era `prompt()`, que webviews Android y el Artifact bloquean.
**Arreglo:** formulario `#yt-agregar` (campo de enlace + "Agregar") arriba de los reproductores, siempre visible; la UI se dibuja de inmediato y los reproductores se preparan en segundo plano con estado "Conectando con YouTube…"; `MotorYouTube.preparar` y `AnalizadorYouTube` con timeout de 30 s y reintento (se limpia `#listo` y se destruyen los players); `UIDJ.importarEnlaces(texto)` público, `prompt()` envuelto en try y con fallback al campo. Verificado en navegador: el campo y la biblioteca aparecen aunque YouTube no responda.

## 25. "Puse una canción, puse otra y se quitó la primera" (2026-09-12) — v3.4.2
**Diagnóstico:** la biblioteca no pierde pistas (ids únicos `pst_<t>_<n>`, prueba con 3 importaciones). Lo que el usuario veía era el reemplazo en el mismo deck (▶A dos veces) o el cambio del reproductor "Vista previa". La interfaz no dejaba claro a dónde va cada video.
**Arreglo:** `DJAPI.colocar(pistaId)` → primer deck vacío (A, luego B) o la cola. En modo YouTube (`autoColocar: true`) cada enlace agregado se coloca solo y aparece un aviso "«Título» → deck A · «Título» → deck B · … → cola" durante 6 s. Texto de ayuda de la biblioteca actualizado. 113/113 pruebas.

## 26. Buscador dentro de YouTube y SYNC entre videos (2026-09-13) — v3.5.0
**Buscador.** Única vía legítima: YouTube Data API v3 con clave del usuario (gratis, 10 000 unidades/día; cada búsqueda cuesta 100 → ~100 búsquedas diarias). Adaptador `src/yt/adaptadores/BuscadorYouTube.js` (search + videos para duraciones; errores traducidos: sin clave, clave inválida, cuota agotada, sin internet) y `UIBuscadorYouTube.js` (campo de búsqueda, resultados con miniatura/canal/duración y "+ Agregar" que va por el mismo camino que un enlace pegado → deck A, B o cola). La clave se guarda en `localStorage` (`cabina.ytApiKey`) y hay instrucciones en la propia pantalla. Sin clave: enlace "Abrir en YouTube ↗" que abre la búsqueda en YouTube para copiar el enlace. Scraping de resultados descartado (ToS y se rompe).
**SYNC.** No funcionaba porque los videos no tienen BPM (el audio no sale de YouTube) y `sincronizar` devolvía 0 en silencio. Ahora: botón **TAP** en cada deck (tap tempo: 4+ toques, ventana de 2 s, promedio de intervalos) → `DJAPI.fijarBpm(deck, bpm)` (40–250, persiste en la biblioteca y actualiza el deck); `sincronizar` lanza un error claro si falta BPM. `MotorYouTube.fijarTasa` pide la tasa exacta (p. ej. 1.04) y comprueba con `getPlaybackRate()`; si el reproductor la redondea usa el paso de 0.25 más cercano. El deck muestra la tasa real aplicada (×1.04). Límite honesto: aun con BPM igual no hay alineación de fase automática (no hay forma de onda); se ajusta de oído con la posición.
115/115 pruebas. Desplegado en producción y Artifact.
